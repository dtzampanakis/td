...
enemy_images = {
    ...
    "boss": pg.image.load("assets/images/enemies/enemy_4.png").convert_alpha()
}
# buttons
...
logo_image = pg.image.load('assets/images/gui/tsoukalas.png').convert_alpha()
...
text_font = pg.font.SysFont("Consolas", 25, bold=True)
large_font = pg.font.SysFont("Consolas", 26)
...
def display_data():
    ...
    screen.blit(logo_image, (c.SCREEN_WIDTH - 125, 390))
    # display data
...
while run:
    ...
    if game_over is False:
        ...
        if world.check_level_complete():
            world.money += c.LEVEL_CONMPLTE_REWARD
            world.level += 1
            level_started = False
            last_enemy_spawn = pg.time.get_ticks()
            world.reset_level()
        ...
        if selected_turret:
            # if a turrent can be upgraded then show the upgrade button
        ...
        if game_outcome == -1:
            draw_text("GAME OVEWR", large_font, "grey0", 310, 230)
        elif game_outcome == 1:
